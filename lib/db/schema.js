...
// 0. SYSTEM SETTINGS
// =============================================
export const pengaturanSistem = pgTable("pengaturan_sistem", {
  kunci: text("kunci").primaryKey(), // e.g., 'ACTIVE_AI_PROVIDER', 'GEMINI_API_KEY', 'OPENAI_API_KEY'
  nilai: text("nilai"), // The value or API key
  deskripsi: text("deskripsi"), // Human readable description
...
export const rpp = pgTable("rpp", {
  ...
  targetKognitif: text("target_kognitif").default("C3"), // C1 - C6
...
export const bahanAjar = pgTable("bahan_ajar", {
  ...
  guruId: integer("guru_id").references(() => guru.id),
  mapelId: integer("mapel_id").references(() => mapel.id),
  rppId: integer("rpp_id").references(() => rpp.id, { onDelete: "set null" }),
  judul: text("judul").notNull(),
...
export const soal = pgTable("soal", {
  ...
  tipe: text("tipe").default("pilihan_ganda"), // pilihan_ganda, essay
  levelBloom: text("level_bloom"), // C1, C2, C3, C4, C5, C6
  opsiA: text("opsi_a"),
  opsiB: text("opsi_b"),
  opsiC: text("opsi_c"),
  opsiD: text("opsi_d"),
  kunciJawaban: text("kunci_jawaban"), // A, B, C, D atau teks essay
  bobot: doublePrecision("bobot").default(1),
...
// =============================================
// 16. KELOMPOK TAHFIZH (HALAQAH)
// =============================================
export const kelompokTahfizh = pgTable("kelompok_tahfizh", {
  id: serial("id").primaryKey(),
  namaKelompok: text("nama_kelompok").notNull(), // Misal: "Halaqah Utsman (Juz 30)"
  guruId: integer("guru_id").references(() => guru.id, { onDelete: "cascade" }),
  tahunAjaranId: integer("tahun_ajaran_id").references(() => tahunAjaran.id),
  deskripsi: text("deskripsi"),
});

export const anggotaTahfizh = pgTable("anggota_tahfizh", {
  id: serial("id").primaryKey(),
  kelompokId: integer("kelompok_id").references(() => kelompokTahfizh.id, { onDelete: "cascade" }),
  siswaId: integer("siswa_id").references(() => siswa.id, { onDelete: "cascade" }),
  tanggalBergabung: text("tanggal_bergabung").default(sql`now()`),
});
