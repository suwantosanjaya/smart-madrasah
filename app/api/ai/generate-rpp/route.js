...
export async function POST(req) {
  try {
    ...
    const kognitifCode = targetKognitif ? targetKognitif.substring(0, 2) : "C3"; // extract C1/C2 etc
    ...
    if (mapel.toLowerCase().includes("matematika")) {
      tujuan = `Peserta didik mampu memahami konsep dasar ${judul} dan memecahkan soal cerita yang berkaitan dengan ${judul} dalam kehidupan sehari-hari secara mandiri dan kritis. (Mencapai tingkat nalar ${targetKognitif}).`;
      ...
      tujuan = `Peserta didik mampu mengidentifikasi struktur, ciri kebahasaan, dan makna dari ${judul} yang dibaca/didengar secara teliti, serta mampu menyajikan gagasan terkait ${judul} lisan/tulis (Target: ${targetKognitif}).`;
      ...
      tujuan = `Peserta didik mampu menjelaskan proses atau konsep dari ${judul} melalui kegiatan eksperimen dengan rasa ingin tahu yang tinggi, dengan tingkat nalar ${targetKognitif}.`;
      ...
      tujuan = `Peserta didik mampu memahami konsep pokok dari materi ${judul} sesuai dengan standar pencapaian fase ini secara komprehensif, ditargetkan mencapai taksonomi ${targetKognitif}.`;
    ...
    if (kognitifCode === "C6") {
       inti += "\n[PBL] Siswa diarahkan untuk menciptakan atau merancang karya (proyek) turunan dari materi ini sebagai tugas akhir/formatif.";
    } else if (kognitifCode === "C5") {
       inti += "\n[Diskusi] Siswa saling mengevaluasi hasil lembar kerja kelompok lain dan memberikan argumen kritis.";
    }
